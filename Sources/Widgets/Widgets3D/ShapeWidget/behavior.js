...
export default function widgetBehavior(publicAPI, model) {
  ...
  publicAPI.makeSquareFromPoints = (point1, point2) => {
    ...
    vec3.subtract(direction, point2, point1);
    const right = model.shapeHandle.getRight();
    const up = model.shapeHandle.getUp();
    let rightComponent = vec3.dot(direction, right);
    let upComponent = vec3.dot(direction, up);
    ...
    return [
      point1[0] + rightComponent * right[0] + upComponent * up[0],
      point1[1] + rightComponent * right[1] + upComponent * up[1],
      point1[2] + rightComponent * right[2] + upComponent * up[2],
    ];
  };
